...
public class Inventory {
	...
	public Instrument get(String serialNumeber){
		for(Iterator<Instrument> i=inventory.iterator();i.hasNext();){
			Instrument instrument=(Instrument)i.next();
			if(instrument.getSerialNumber().equals(serialNumeber)){
				return instrument;
			}
		}
		return null;
	...
	public List<Instrument> search(InstrumentSpec otherSpec){
		...
		for(Iterator<Instrument> iter=inventory.iterator();iter.hasNext();){
